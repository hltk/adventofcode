...
class IntCode:
	...
	def run(self, addinp):
		...
		if self.mem[self.pc] == 99:
			self.halted = True
		return self.out
L = itertools.permutations(range(5))
z = ints(open('input').readlines()[0])
u = 0
for perm in L:
	prev_out = [0]
	comps = [IntCode(z) for _ in range(5)]
	for x in range(5):
		comps[x].inp.append(x + 5)
	for x in itertools.cycle(perm):
		prev_out = comps[x].run(prev_out)
		if not prev_out:
			break
		u = max(u, max(prev_out))
print(u)
